...
hotel_llm = ChatOpenAI(api_key=OPENAI_API_KEY, model="gpt-4-turbo", temperature=0.7)
travel_llm = ChatOpenAI(api_key=OPENAI_API_KEY, model="gpt-4-turbo", temperature=0.7)
food_llm = ChatOpenAI(api_key=OPENAI_API_KEY, model="gpt-4-turbo", temperature=0.7)
event_llm = ChatOpenAI(api_key=OPENAI_API_KEY, model="gpt-4-turbo", temperature=0.7)
response_llm = ChatOpenAI(api_key=OPENAI_API_KEY, model="gpt-4-turbo", temperature=0.7)
...
# 유틸리티 함수들
def extract_profile_simple(message: str) -> Dict:
...
async def extract_profile_info_original(message: str, current_profile: UserProfile) -> Dict:
    """메시지에서 프로필 정보 추출"""
...
검색 쿼리:"""
    ...
    print(f"🚀 병렬 검색 시작! (시작 시간: {search_start_time:.3f})")
    ...
    for i, (category, task) in enumerate(tasks.items()):
        ...
        if isinstance(result, Exception):
            print(f"❌ {category} 병렬 검색 실패: {result}")
            results[category] = []
        else:
...
class SmartJejuChatbot:
    ...
    async def stream_chat(self, user_message: str, session_id: str = None):
        """🌊 실시간 스트리밍 채팅"""
        if session_id:
        ...
        try:
            ...
            # 각 노드 실행을 스트리밍으로 처리
            yield {"content": "🤔 사용자 프로필을 분석 중...", "agent": "profile_collector"}
            
            # 프로필 수집 단계
            profile_result = await profile_collector_node(state)
            if not profile_result.get("profile_ready", False):
                # 추가 정보가 필요한 경우 즉시 응답
                response_text = profile_result.get("final_response", "")
                for token in response_text.split():
                    yield {"content": token + " ", "agent": "profile_collector"}
                    await asyncio.sleep(0.05)  # 토큰 간 딜레이
                return
            
            # 프로필이 준비된 경우 병렬 검색 실행
            yield {"content": "\n\n🔍 여행 정보를 검색 중...", "agent": "search"}
            
            # 병렬 검색 실행
            search_result = await parallel_search_all(profile_result)
            
            # 각 에이전트 결과를 스트리밍으로 표시
            agents = ["숙박", "관광", "음식", "이벤트"]
            for agent in agents:
                yield {"content": f"✅ {agent} 정보 수집 완료 ", "agent": "search"}
                await asyncio.sleep(0.2)
            
            yield {"content": "\n\n📝 최종 추천을 생성 중...", "agent": "response_generator"}
            
            # 최종 응답 생성 및 스트리밍
            final_result = await response_generator_node(search_result)
            ...
            for i, word in enumerate(words):
                # 문장 단위로 약간의 지연 추가
                if word.endswith(('.', '!', '?')):
                    yield {"content": word + " ", "agent": "response_generator"}
                ...
                # 중간 단계 피드백
                if i % 20 == 0 and i > 0:
                    yield {"content": "", "agent": "heartbeat", "progress": f"{i}/{len(words)}"}
            
        except Exception as e:
            yield {"content": f"❌ 오류가 발생했습니다: {str(e)}", "agent": "error"}
...
async def stream_chat(request: ChatRequest):
    ...
    async def generate_stream():
        try:
            # 스트리밍 시작 신호
            yield f"data: {json.dumps({'type': 'start', 'content': '', 'timestamp': datetime.now().isoformat()})}\n\n"
            
            # 멀티에이전트 스트리밍 실행
            async for chunk in chatbot.stream_chat(request.content, request.session_id):
                # 각 토큰을 즉시 전송
                chunk_data = {
                    'type': 'token',
